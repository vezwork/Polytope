...
import { processForceGraph, processGraph, stringToEditor } from "./string_to_editor.js";
import { builder } from "./stringToEditorBuilder.js";
import { generatorPaths, multiplicationTable, cycleGraph } from "./group_theory.js";
    ...
    constructor({ code = [] } = {}) {
        ...
        this.addEventListener('keydown', (e) => {
            ...
            if (this.isCaretInSlot) {
                ...
                this.code.splice(this.caret, 0, new CustomDropdown({ parentEditor: this }));
                const focuser = this.moveCaret(1);
                this.codeEl.innerHTML = '';
                this.codeEl.append(...this.displayHTML());
                if (focuser) {
    ...
    fromInput(input) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (nodeValue instanceof HTMLElement) {
                editor = new TextEditorElement({ code: [nodeValue], parentEditor: this });
            } else {
                editor = new TextEditorElement({ code: [String(nodeValue)], parentEditor: this });
            }
            editor.style.position = 'absolute';

            this.nodes[i] = { x: position[0], y: position[1], editor, adjacent: [] };
            this.shadowRoot.append(editor);
        }
        for (let i = 0; i < nodes.length; i++) {
...
export class ColoredGraphEditorElement extends EditorElement {
    ...
    constructor() {
        ...
        this.addEventListener('keydown', (e) => {
            if (e.key === "Backspace" && this.parentEditor) {
                ...
                this.style.setProperty('--editor-color', GRAPH_COLORS[this.currentColor]);
            }
        });
        this.addEventListener('subEditorDeleted', (e) => {
            this.nodes = this.nodes.filter(({ editor }) => editor !== e.detail);
            for (const node of this.nodes) {
                for (let i = 0; i < GRAPH_COLORS.length; i++) {
                    node.adjacent[i] = node.adjacent[i].filter(({ editor }) => editor !== e.detail);
                }
            }
            this.shadowRoot.removeChild(e.detail);
            this.render();
            this.focus();
            this.parentEditor?.dispatchEvent(new CustomEvent(
                'childEditorUpdate',
                {
                    detail: {
                        out: this.getOutput(),
        ...
        this.addEventListener('mousedown', (e) => {
            const editor = new StringEditorElement({ parentEditor: this, code: String(this.nodes.length).split('') });
            editor.style.position = 'absolute';
            const node = {
                x: e.offsetX - 10,
                y: e.offsetY - 10,
        ...
        this.addEventListener('mouseup', (e) => {
            ...
            if (targetNode) {
                if (this.fromNode && this.fromNode !== targetNode && !this.fromNode.adjacent[this.currentColor].includes(targetNode)) {
    ...
    fromInput(input) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            let editor;
            if (nodeValue instanceof HTMLElement) {
                editor = new StringEditorElement({ code: [nodeValue], parentEditor: this });
            } else {
                editor = new StringEditorElement({ code: [String(nodeValue)], parentEditor: this });
            }
            editor.style.position = 'absolute';

            this.nodes[i] = {
                x: position[0], y: position[1], editor, adjacent: {
                    '0': [],
                    '1': [],
                    '2': [],
                    '3': []
    ...
    getOutput() {
        ...
        return `(${JSON.stringify({ nodes: nodes.map(node => node.getOutput().split("\"").join('')), edges, positions, isColoredGraph: true }, null, 2)})`;
    }
}
    ...
    constructor() {
        ...
        const move = () => {
            ...
            for (let i = 0; i < this.nodes.length; i++) {
                ...
                const start = [node.x + node.editor.offsetWidth / 2, node.y + node.editor.offsetHeight / 2];
    ...
    class C extends TextEditorElement {
        ...
        constructor() {
            ...
            this.editorEls = editorDescriptions.map(({
                ...
                editorEl.innerHTML = (iconPath ? `<img src="${iconPath}" height="32"> ` : '') + `${name}
${description}`;
                editorEl.addEventListener('click', () => {
                    if (this.parentEditor) {
                        this.parentEditor.dispatchEvent(new CustomEvent(
                            'subEditorReplaced',
                            {
                                detail: { old: this, new: new ElementConstructor({ parentEditor: this.parentEditor }) }
                            }
                        ));
                    }
                });
                    ...
                    if (this.parentEditor) {
                        this.parentEditor.dispatchEvent(new CustomEvent(
                            'subEditorReplaced',
                            {
                                detail: {
                                    old: this,
                                    new: new editorDescriptions[this.selection].ElementConstructor({ parentEditor: this.parentEditor })
                                }
                            }
                        ));
                    }
...
const CustomDropdown = DropdownElement([
    ...
    },
    {
        ...
        ElementConstructor: ColoredGraphEditorElement,
        iconPath: "./icon_cgraph.png"
    },
    {
        name: 'text <-> math',
        ElementConstructor: BidirectionalEditorPair({
            name: 'text-math',
            leftEditorFactory: (string, parentEditor) => new TextEditorElement({
                code: string.split(''),
                parentEditor
            }),
    ...
    },
    {
        ...
        ElementConstructor: UnidirectionalEditorPair({
            ...
            rightEditorFactory: (json, parentEditor) => {
                return new ForceGraphEditorElement({
                    ...json,
                    parentEditor
                })
            }
    ...
    },
    {
        name: 'test',
        description: 'ok',
        ElementConstructor: ConstructiveUnidirectionalEditor({
            name: 'test',
            leftEditorFactory: (string, parentEditor) => new TextEditorElement({
                code: string,
                parentEditor
