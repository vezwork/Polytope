...
export type FocusFromParentEditorArgs = {
  direction: EnterEditorDirection;
};
...
export class EditorElement extends HTMLElement {
  ...
  builder?: (input: string) => { output: EditorElement };
  selectionStartEditor: EditorElement | null = null;
        ...
        if (rootEditor.selectionStartEditor) {
          ...
          for (const editor of rootEditor.childEditors()) {
            if (editor === rootEditor.selectionStartEditor || editor === this) {
              if (editor.childEditors().length === 0) editor.makeSelected();

              if (rootEditor.selectionStartEditor === this) {
                isInLinearSelection = false;
              } else {
                if (isInLinearSelection) this.makeFocused();
  ...
  clearChildSelections() {
    ...
    for (const editor of this.childEditors()) editor.makeUnselected();
  }

  focusFromChildEditor(args: FocusFromChildEditorArgs): void {
  ...
  focusFromParentEditor(args: FocusFromParentEditorArgs): void {
  ...
  makeFocused() {
    ...
    if (this.parentEditor) this.parentEditor.makeUnfocused();
  }
  makeUnfocused() {
