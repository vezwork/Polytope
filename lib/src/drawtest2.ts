...
import { add, distance, sub, Vec2 } from "./math/Vec2.js";
...
const φ = centerBesideD(
  tD("1 + "),
  editor(centerOverD(tD("1"), slad(placeholder)))
);
...
const { descendentsBreadthFirst: des } = makeTreeFunctions<DrawTree>({
  parent,
  children,
});
...
const main = editor(
  justOverD(
    padD([0, 120])(justBesideD(tD("φ = "), transformD(id)(φ))),
    centerBesideD(
      tD("ζ(s)﹦"),
      ΣNote,
      tD("﹦"),
...
function anim() {
  ...
  ctx.lineWidth = 2;
  canvasRender(
    translateD([x - 4 + width, y - 4])(
      scaleD([height + 8, height + 8])(textD(...measureWidth("🥕", 1)))
    )
  );
  ctx.lineWidth = 1;
