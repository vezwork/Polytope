...
export class ColoredGraphEditorElement extends EditorElement {
    ...
    getOutput() {
        ...
        return `(${JSON.stringify({ nodes: nodes.map(node => node.getOutput().split("\"").join('')), edges, positions, isColoredGraph: true })})`;
    }
}
...
export class ForceColoredGraphEditorElement extends EditorElement {
    ...
    fromInput(input) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            this.nodes[i] = {
                x: 100 + i, y: 100 + i, editor, adjacent: {
                    '0': [],
                    '1': [],
                    '2': [],
                    '3': []
...
customElements.define('force-color-graph-editor', ForceColoredGraphEditorElement);
