...
export class EditorElement extends HTMLElement {
    ...
    childBelow(childEditor, carryX) {
        ...
        const boundsBelow = boundsChildEditors.filter(({ editor, bounds }) => boundsChild.bottom < bounds.top && editor !== childEditor);
        const closestBelow = boundsBelow.sort((a, b) => yBiasedManhattanDist([carryX ?? boundsChild.right, boundsChild.bottom], closestPointOnBounds([carryX ?? boundsChild.right, boundsChild.bottom], a.bounds)) -
            yBiasedManhattanDist([carryX ?? boundsChild.right, boundsChild.bottom], closestPointOnBounds([carryX ?? boundsChild.right, boundsChild.bottom], b.bounds)));
        return closestBelow[0]?.editor ?? null;
    }
    childAbove(childEditor, carryX) {
        // mild spatial nav
        const boundsChild = childEditor.getBoundingClientRect();
        const boundsChildEditors = this.childEditors().map((editor) => ({
            editor,
        ...
        const boundsAbove = boundsChildEditors.filter(({ editor, bounds }) => boundsChild.top > bounds.bottom && editor !== childEditor);
        const closestAbove = boundsAbove.sort((a, b) => yBiasedManhattanDist([carryX ?? boundsChild.right, boundsChild.top], closestPointOnBounds([carryX ?? boundsChild.right, boundsChild.top], a.bounds)) -
            yBiasedManhattanDist([carryX ?? boundsChild.right, boundsChild.top], closestPointOnBounds([carryX ?? boundsChild.right, boundsChild.top], b.bounds)));
    ...
    closestChildOrThisToPosition(pos) {
        const closestChild = this.childEditors().sort((a, b) => {
            const aBound = a.getBoundingClientRect();
            const bBound = b.getBoundingClientRect();
            return (yBiasedManhattanDist(pos, closestPointOnBounds(pos, aBound)) - // should really be dist to the entire right vertical segment
                yBiasedManhattanDist(pos, closestPointOnBounds(pos, bBound)));
        })[0];
        const myBound = this.getBoundingClientRect();
        const distToClosestChild = yBiasedManhattanDist(pos, closestPointOnBounds(pos, closestChild.getBoundingClientRect()));
        const distToRightSideOfThis = yBiasedManhattanDist(pos, closestPointOnBounds(pos, {
            top: myBound.top,
            right: myBound.right,
    ...
    parentFocusFromParentEditor(args) {
        ...
        if (direction === EnterEditorDirection.up) {
            ...
            if (a === this)
                ...
                a.focusFromParentEditor({ direction, x }); // TODO: fix for carryX
        }
