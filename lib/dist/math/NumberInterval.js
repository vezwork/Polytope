...
export const orderIncreasing = ([n1, n2]) => [Math.min(n1, n2), Math.max(n1, n2)];
export const size = (i) => {
...
export const isInInterval = (n, i) => {
    const [min, max] = orderIncreasing(i);
    return n >= min && n <= max;
};
export const seperatingInterval = (a, b) => {
    const [min1, max1] = orderIncreasing(a);
    const [min2, max2] = orderIncreasing(b);
    if (min1 > max2)
        return [max2, min1];
...
export const overlappingInterval = (a, b) => {
    const [min1, max1] = orderIncreasing(a);
    const [min2, max2] = orderIncreasing(b);
    if (isInInterval(min1, b))
        return [min1, Math.min(max1, max2)];
    if (isInInterval(min2, a))
        return [min2, Math.min(max1, max2)];
    if (isInInterval(max1, b))
        return [Math.max(min1, min2), max1];
    if (isInInterval(max2, a))
        return [Math.max(min1, min2), max2];
    return null;
};
// signed seperation
...
export const seperation = (a, b) => {
    const d = dist(a, b);
    if (d < 0)
        return 0;
    else
        return Math.min(...a) > Math.max(...b) ? d : -d;
};
export const areSeperated = (a, b) => dist(a, b) < 0;
export const areOverlapped = (a, b) => dist(a, b) > 0;
...
// ---------------------> n-axis
//
export const axisAlignedIntervalDist = (a, b) => {
    const i = seperatingInterval(a.interval, b.interval);
    if (i === null)
        return Math.abs(a.n - b.n); // intervals overlap so just get 1D distance
