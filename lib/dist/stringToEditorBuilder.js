...
export const createBuilder = (processors = []) => {
    const f = (string, hasOpenParen = false) => {
        ...
        for (let i = 0; i < string.length; i++) {
            const char = string[i];
            if (char === "(") {
                const { consume, output } = f(string.substring(i + 1), processors
                //true
                );
...
export const createJSONProcessor = (editorFactory, validator) => (innerString, result) => {
    try {
        ...
        const processedJSON = objectValueMap(innerJSON, (value) => value === EDITOR_IDENTIFIER ? innerEditors.pop() : value);
        if (!validator(processedJSON))
            return false;
...
export const createFunctionProcessor = ({ name, arity, ElementConstructor }) => (innerString, result, string, i) => {
