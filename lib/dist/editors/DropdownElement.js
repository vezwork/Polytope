...
export const DropdownElement = (editorDescriptions, name = "no-name") => {
    class C extends TextEditorElement {
        constructor() {
            ...
            this.editorEls = editorDescriptions.map(({ name, description, iconPath, ElementConstructor }) => {
  ...
  ${description}`;
                editorEl.addEventListener("click", () => {
                    if (this.parentEditor) {
                        this.parentEditor.dispatchEvent(new CustomEvent("subEditorReplaced", {
                            detail: {
                                old: this,
            ...
            this.addEventListener("keydown", (e) => {
                ...
                for (const [editorEl, i] of withIndex(this.editorEls)) {
                    if (i === this.selection)
                        editorEl.style.background = "#ffd608";
                    else
                        editorEl.style.background = "#FEFEFE";
                }
                if (e.key === "Enter") {
                    if (this.parentEditor) {
                        this.parentEditor.dispatchEvent(new CustomEvent("subEditorReplaced", {
