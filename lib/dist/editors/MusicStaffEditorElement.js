...
export class MusicStaffEditorElement extends ArrayEditorElement {
    ...
    render() {
        ...
        for (const { note, length } of this.contents) {
            const y = STAFF_BOTTOM_NOTE_Y - (NOTES.indexOf(note) * STAFF_LINE_HEIGHT) / 2;
            if (note === " ") {
                notesWrapperEl.append(makeSVGEl("use", { href: "#quarter-rest", x, y: 11, i }));
            }
            else if (y < 10) {
                notesWrapperEl.append(makeSVGEl("use", { href: "#quarter-note-flipped", x, y: y + 10.5, i }));
            }
            else {
                notesWrapperEl.append(makeSVGEl("use", { href: "#quarter-note", x, y, i }));
            }
            if (i !== 0 && i % 4 === 0) {
                notesWrapperEl.append(makeSVGEl("use", { href: "#vertical-line", x: x - 3 }));
            }
            x += 14;
            i++;
        }
        ...
        notesWrapperEl.append(makeSVGEl("use", { href: "#caret", x: 19.5 + this.caret * 14, y: 5 }));
        const [start, end] = this.caretsOrdered();
        if (start !== end) {
